$(document).ready(function(){
	...
	function get_komoditas1(){
		...
		var kecamatan_id = $(".all-districts-hutan").find(":selected").attr("value");
		var kelurahan_id = $(".all-subdistricts-hutan").find(":selected").attr("value");
		...
		$.post(config_url + "panel/hutan/json/get-hutan",data,function(rd){
			var bar_data = {
				data : [],
				bars : {show:true},
			}
			...
			for(var i=0;i<rd['jenis_komoditas'].length;i++){
				...
				tick.push(i,rd['jenis_komoditas'][i]['nama_komoditas_kehutanan']);
		    ...
		    for(var is=0;is<rdm.length;is++){
			    text += "<tr><td>"+(parseInt(is)+1)+"</td><td>"+rdm[is]['nama_komoditas_kehutanan']+"</td><td>"+rdm[is]['total_data']+"</td></tr>";
			}

		    $("#jumlah-kk-hutan tbody").html(text);
		},"json")
		.fail(function(rd){
			console.log(rd);
	...
	$(".all-districts-hutan").on("change",function(){
		get_kelurahan_hutan();
	});

	function get_kelurahan_hutan(){
		var kecamatan_id = $(".all-districts-hutan").find(":selected").attr("value");
		var page_csrf = $(".csrf-header-master").attr("name");
		var page_csrf_value = $(".csrf-header-master").attr("value");
		...
		$.post(config_url + "panel/address/json/get-subdistrict/"+kecamatan_id,data,function(rd){
			...
			$(".all-subdistricts-hutan").html(text);
		},"json")
		.fail(function(rd){
			console.log(rd);
			Toast.fire({
	...
	$(".filter-hutan").on("click",function(){
		get_komoditas1();
	});
