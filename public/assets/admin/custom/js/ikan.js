$(document).ready(function(){
	...
	function get_komoditas1(){
		...
		var kecamatan_id = $(".all-districts-ikan").find(":selected").attr("value");
		var kelurahan_id = $(".all-subdistricts-ikan").find(":selected").attr("value");
		...
		$.post(config_url + "panel/ikan/json/get-ikan",data,function(rd){
			var bar_data = {
				data : [],
				bars : {show:true},
			}
			...
			for(var i=0;i<rd['jenis_komoditas'].length;i++){
				...
				tick.push(i,rd['jenis_komoditas'][i]['nama_komoditas_ikan']);
	...
	$(".all-districts-ikan").on("change",function(){
		get_kelurahan_ikan();
	});

	function get_kelurahan_ikan(){
		var kecamatan_id = $(".all-districts-ikan").find(":selected").attr("value");
		var page_csrf = $(".csrf-header-master").attr("name");
		var page_csrf_value = $(".csrf-header-master").attr("value");
		...
		$.post(config_url + "panel/address/json/get-subdistrict/"+kecamatan_id,data,function(rd){
			...
			$(".all-subdistricts-ikan").html(text);
		},"json")
		.fail(function(rd){
			console.log(rd);
			Toast.fire({
