$(document).ready(function(){
	...
	function get_komoditas1(){
		...
		var kecamatan_id = $(".all-districts-ternak").find(":selected").attr("value");
		var kelurahan_id = $(".all-subdistricts-ternak").find(":selected").attr("value");
		...
		$.post(config_url + "panel/peternak/json/get-peternak",data,function(rd){
			var bar_data = {
				data : [],
				bars : {show:true},
			}
			...
			for(var i=0;i<rd['jenis_ternak'].length;i++){
				ar = [];
				tick = [];
				ar.push(i,rd['jenis_ternak'][i]['total_data']);
				tick.push(i,rd['jenis_ternak'][i]['nama_komoditas_ternak']);
	...
	$(".all-districts-ternak").on("change",function(){
		get_kelurahan_ternak();
	});

	function get_kelurahan_ternak(){
		var kecamatan_id = $(".all-districts-ternak").find(":selected").attr("value");
		var page_csrf = $(".csrf-header-master").attr("name");
		var page_csrf_value = $(".csrf-header-master").attr("value");
		...
		$.post(config_url + "panel/address/json/get-subdistrict/"+kecamatan_id,data,function(rd){
			...
			$(".all-subdistricts-ternak").html(text);
		},"json")
		.fail(function(rd){
			console.log(rd);
			Toast.fire({
	...
	$(".filter-ternak").on("click",function(){
		get_komoditas1();
	});
